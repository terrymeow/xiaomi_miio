...
SWITCH_TYPES = (
    ...
    XiaomiMiioSwitchDescription(
        key=ATTR_LED_LIGHT,
        feature=FEATURE_SET_LIGHT,
        name="Led",
        icon="mdi:led-outline",
        method_on="async_set_light_on",
        method_off="async_set_light_off",
        entity_category=EntityCategory.CONFIG,
    ),
    XiaomiMiioSwitchDescription(
        key=ATTR_LEARN_MODE,
...
class XiaomiGenericCoordinatedSwitch(XiaomiCoordinatedMiioEntity, SwitchEntity):
    ...
    async def async_set_light_on(self) -> bool:
        """Turn the led on."""
        return await self._try_command(
            "Turning the led of the miio device on failed.",
    ...
    async def async_set_light_off(self) -> bool:
        """Turn the led off."""
        return await self._try_command(
            "Turning the led of the miio device off failed.",
            self._device.set_light,
